...
public class BookComposite(int id, string name) : IComponent
{
    ...
    public int Count() => _components.Sum(x => x.Count());
    ...
    public string Display()
    {
        ...
        foreach (var item in _components)  sb.Append(item.Display());
    ...
    public List<SelectListItem> GetSelectListItems(string line)
    {
        ...
        if (_components.Any(x => x is BookComposite)) line += " - "; 

        _components.ForEach(x =>
        {
            if (x is BookComposite bookComposite) list.AddRange(bookComposite.GetSelectListItems(line));
        });
        return list;
    }
}
