...
using WebApp.Composite.DP.Composite;
using WebApp.Composite.DP.Models;

namespace WebApp.Composite.DP.Controllers;

[Authorize]
public class CategoryMenuController(AppIdentityDbContext context) : Controller
{
        ...
        categories.Where(x => x.ReferenceId == topCategory.Id).ToList().ForEach(categoryItem =>
           {
               ...
               categoryItem.Books.ToList().ForEach(bookItem => bookComposite.Add(new BookComponent(bookItem.Id, bookItem.Name)));

               if (last != null) last.Add(bookComposite);
               else topBookComposite.Add(bookComposite);
